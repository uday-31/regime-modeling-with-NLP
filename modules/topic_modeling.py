...
class TopicModel():
    ...
    def __init__(self, tfidf_mat, dictionary, statements, crossval=True, num_topics_list=[2, 5, 7, 10], num_topic=10, cv_score="c_v"):
        """_summary_
        Constructor
        Args:
    ...
    def model(self):
        if self.crossval:
            ...
            self.num_topic = self.cross_val(
                n_splits=5)
        self.cv_model = Nmf(corpus=self.tfidf_mat, id2word=self.dictionary,
                            num_topics=self.num_topic, random_state=42)
    ...
    def cross_val(self, n_splits=5):
        ...
        for num_topics in self.num_topics_list:
            ...
            for train_idx, _ in kf.split(self.tfidf_mat):
                ...
                model = Nmf(train_stmts, num_topics=num_topics,
                            id2word=self.dictionary, passes=5, random_state=42)
                coherence_model = CoherenceModel(
                    model=model, texts=self.statements, dictionary=self.dictionary, coherence=self.cv_score)
